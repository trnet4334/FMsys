...
const STATUSES = ['Cleared', 'Pending'] as const;

type AddInflowModalProps = {
  onClose: () => void;
};

export function AddInflowModal({ onClose }: AddInflowModalProps) {
      ...
      <div className="relative w-full max-w-lg bg-card border-t md:border border-line md:rounded-2xl shadow-2xl flex flex-col max-h-[90dvh] animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="px-6 h-16 flex items-center justify-between border-b border-line shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-ink-0 text-lg">New Inflow</h2>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-[10px] font-bold text-ink-disabled uppercase tracking-widest">Source</label>
              ...
              {SOURCES.map(({ key, Icon, label }) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border-2 transition-all text-[10px] font-bold ${
                    source === key
                      ? 'bg-brand-weak border-brand text-brand shadow-[0_0_12px_rgba(0,102,255,0.15)]'
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g. Monthly Salary – TechCorp"
          ...
          {/* Note (optional) */}
          <div className="space-y-2">
            <label className="block text-[10px] font-bold text-ink-disabled uppercase tracking-widest">Note <span className="normal-case font-normal">(optional)</span></label>
            <textarea
              value={note}
        ...
        <div className="px-6 py-4 border-t border-line bg-bg-1/50 flex flex-col-reverse md:flex-row gap-3 justify-end shrink-0">
          ...
          <button
            type="button"
            className="w-full md:w-auto px-8 py-3 bg-emerald-500 text-white font-black rounded-xl shadow-soft hover:opacity-90 active:scale-[0.98] transition-all text-sm"
          >
            Add Inflow
          </button>
        </div>
